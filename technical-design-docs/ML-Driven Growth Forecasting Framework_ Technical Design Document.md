...
This document describes the design and implementation of an end-to-end machine learning forecasting framework built to predict daily user growth metrics, specifically First-Time User (FTU) activations and Monthly Active Users (MAU), across four credit and payment products operating in two international markets (United States and United Kingdom).
...
The framework I designed and built addresses these gaps through several interconnected innovations:
...
\- \*\*A cross-validated candidate model benchmarking framework\*\* that evaluates 20+ candidate algorithms (linear models, tree-based ensembles, gradient-boosted machines, statistical time-series methods, and deep learning sequence models) and selects the optimal model per product-metric combination through time-ordered cross-validation and information-criterion-guided ranking.  
\- \*\*A deep-learning-first design\*\* in which neural sequence models (N-BEATS, a Temporal Fusion Transformer variant, and DeepAR) are first-class participants in model selection. Deep learning is treated as crucial, not optional: these architectures provide the representational capacity required to capture long-range dependencies, nonlinear feature interactions, and native probabilistic forecast distributions that classical models cannot.  
\- \*\*An ensemble meta-learning layer\*\* (v2) that combines top-performing models via inverse-error weighting, stacking with out-of-fold predictions, and blending to reduce variance and improve robustness.  
\- \*\*A domain-constrained prediction pipeline\*\* that applies activation rate bounds, holiday penalty adjustments, historical range clipping, trend-aware caps, monotonicity enforcement, and recursion-chain stabilization to ensure forecasts remain within operationally feasible ranges.  
...
The system achieved a 23% improvement in FTU forecast accuracy and an 18% improvement in MAU forecast accuracy relative to the prior manual baselines, as measured by out-of-sample RMSE on held-out time-ordered validation sets.
...
Growth forecasting for consumer credit products presents a distinct set of challenges that differentiate it from standard demand forecasting or revenue prediction:

1\. \*\*Heterogeneous user lifecycle dynamics.\*\* New user activations, reactivations of dormant accounts, and churn follow fundamentally different statistical processes. Activations correlate with marketing spend and product launches; reactivations respond to engagement campaigns and seasonal triggers; churn exhibits regime-switching behavior driven by macroeconomic conditions and competitive dynamics. A single model architecture cannot adequately capture all three without explicit structural accommodation.

2\. \*\*Multi-product, multi-market variation.\*\* Each product-market combination exhibits distinct seasonality patterns, trend characteristics, and response to exogenous variables. A US-market co-branded credit card shows different holiday sensitivity than a UK-market payment product. Legacy product cohorts lack certain metrics entirely (e.g., no new activation data for sunset cohorts). The framework must handle this heterogeneity without requiring per-product custom code.
...
5\. \*\*Campaign-driven demand shocks.\*\* Promotional campaigns (welcome bonuses, limited-time offers, partner co-marketing events) produce sharp, non-recurring spikes in activation and engagement that contaminate the trend-plus-seasonality decomposition assumed by classical methods. Forecasting the underlying baseline requires explicit modeling of campaign contributions as exogenous covariates rather than absorbing them into residual noise.
...
\- Exponential smoothing (ETS/Holt-Winters) handles trend and seasonality but cannot incorporate exogenous regressors (transaction volume, approval rates, marketing signals) that carry significant predictive power for user growth.  
\- Both families assume a single data-generating process. The multi-metric structure (activations, reactivations, churn) requires either separate univariate models with no cross-metric information sharing, or a VAR-type multivariate extension that introduces parameter explosion on short series.
...
\- \*\*Interaction and higher-order features (v2):\*\* Fourier terms at yearly, quarterly, and monthly frequencies; piecewise-linear trend segments with data-driven changepoints; pairwise and triple feature interactions between campaign intensity, holiday proximity, and day-of-week, yielding approximately 90 features in the expanded v2 pipeline.
...
7\. \*\*Hard circuit-breaker caps.\*\* An absolute floor and ceiling act as a safety net: predictions outside these bounds are clipped and flagged for manual review. In normal operation these caps rarely activate (fewer than 5% of predictions) but they prevent catastrophic failure modes in which a single rogue prediction poisons the recursion chain.  
...
\*\*Deployment Layer.\*\* The deployment layer is intentionally thin, which is a design choice, not a simplification. The work that would normally live in deployment tooling (idempotency, schema evolution, backfill logic) is instead encoded in a schema contract and an artifact registry that sit between the forecasting engine and the cloud warehouse. Concretely, the deployment layer performs: artifact registration (the run produces a versioned forecast artifact with run metadata, model identity, and feature-set hash), schema contract validation against the target warehouse table, partitioned idempotent write to the warehouse via bulk load of parquet shards keyed by date partition, and automated verification queries that reconcile row counts, date ranges, and aggregate totals against the engine's in-memory outputs. There is no CSV staging step and no manual copy; the engine writes directly to cloud storage and issues the warehouse load in a single atomic operation per partition.

\#\#\# 3.3 Configuration-Driven Design

A central design principle is that \*\*product-specific behavior is driven entirely by configuration, not by code branching.\*\* Each product is defined by a YAML configuration specifying:
...
This configuration-driven approach enables adding new products or markets by writing a YAML block and registering an input query. No library code changes are required. The same codebase serves all product-market combinations.
...
| \*\*Key exogenous signal\*\* | Monthly approval volume (distributed to daily) | Transaction volume (TPV) interpolated to daily |  
...
In this framework, \*\*XO\*\* refers to the Checkout form factor (the PayPal checkout product surface used on merchant pages) and \*\*Non-XO\*\* refers to all Non-Checkout form factors (wallet, send money, in-app product surfaces). The two surfaces exhibit materially different user behavior patterns, so they are modeled independently and the resulting predictions are summed to produce the total product-level MAU.

This dual architecture reflects a deliberate design decision: rather than forcing both metrics into a single model structure, each subsystem is optimized for its specific data-generating process while sharing infrastructure (validation, feature engineering, model selection, deployment).
...
The framework employs a \*\*cross-validated candidate benchmarking\*\* strategy rather than committing to a single algorithm family. This decision was driven by empirical observation during development: no single model family consistently outperformed across all product-metric combinations. Linear models excelled on products with strong trend components and limited seasonality. Tree-based ensembles captured non-linear interactions in products with complex feature relationships. Gradient-boosted machines achieved the best balance for products with moderate data volumes. Deep sequence models captured long-range dependencies and nonlinear feature interactions on products with longer histories.

Rather than performing expensive nested hyperparameter optimization, the framework evaluates a curated set of model configurations that span the bias-variance tradeoff:
...
The v2 architecture extends the candidate pool to 20+ models through a plugin-based registry system:

\*\*Statistical Time-Series Models:\*\*  
\- \*\*Prophet:\*\* Additive decomposition of trend, yearly and weekly seasonality, and holidays, with configurable changepoint prior scale (0.05). Chosen for its interpretable components and robust handling of missing data.  
...
1\. \*\*Weighted Average Ensemble.\*\* Model weights are proportional to the inverse of their cross-validation RMSE: w\_i \= (1/RMSE\_i) / sum(1/RMSE\_j). This gives higher weight to more accurate models while still incorporating diversity from less accurate but complementary models.
...
3\. \*\*Blending Ensemble.\*\* Simple arithmetic mean of top-K model predictions. Despite its simplicity, blending often performs competitively because it maximally reduces variance without introducing meta-learner estimation error.
...
1\. \*\*Artifact discovery.\*\* The deployment step scans the run registry for the latest versioned forecast artifact matching the target run pattern (\`Q{n}\_{year}\` for reforecast, \`{n}\_months\_forward\_{year}\` for budget). Each artifact carries a content hash, model identity, feature-set hash, and run metadata.  
...
3\. \*\*Schema transformation.\*\* FTU outputs are unpivoted from wide format (one column per metric) to long format (metric\_name column) to match the warehouse schema. MAU outputs are loaded directly.  
4\. \*\*Partitioned idempotent write.\*\* The engine writes parquet shards directly to cloud object storage, keyed by date partition. The warehouse load then replaces each target partition atomically via a single \`DELETE partition \+ LOAD partition\` transaction, so reruns produce byte-identical target state.  
...
Model selection uses \*\*Time Series Split (TSCV)\*\* cross-validation, never random k-fold, to respect the time ordering of observations. This is the single most important methodological choice in the framework.
...
\- \*\*MAE (Mean Absolute Error):\*\* Complementary measure that is more robust to outliers. Useful for communicating "average daily error" to business stakeholders.  
...
2\. \*\*Residual-based intervals.\*\* For computational efficiency, a lighter-weight alternative computes prediction intervals from the distribution of cross-validation residuals: CI \= y\_hat \+/- z\_{alpha/2} \* sigma\_residuals.
...
| \*\*TPV\*\* | Total Payment Volume, aggregate transaction dollar volume, used as an exogenous predictor for MAU |  
...
\*This document describes a methodology and system architecture. No proprietary business data, internal system identifiers, or confidential performance metrics are disclosed. Product category names (credit card, payment product) are used generically. All accuracy improvement figures are relative measures derived from sanitized backtesting comparisons.\*  
